...
class TransformedDataset(Dataset[T_co]):
    def __init__(self, dataset: Dataset, rank0_print=print, data_args=None):
    ...
    def __getitem__(self, index: SupportsIndex) -> T_co:
        ...
        language_raw = self._dataset.meta.episodes[item['episode_index']]["language_dict"]['language_raw']

        if self.data_args.use_reasoning:
            none_counter = 0
            for k in ['substep_reasonings', 'reason']:
                vals = self._dataset.meta.episodes[item['episode_index']]["language_dict"][k]
                if vals is not None:
                    if k == 'substep_reasonings':
                        sub_reasoning = vals[item['frame_index']]
                    else:
                        sub_reasoning = vals
                # else:
            ...
            sub_reasoning = 'Next action:'
        if self.data_args.use_reasoning:
            item['task'] = sub_reasoning
        else:
            item['task'] = language_raw
        # if self.transformations is not None:
        #     image_keys = self._dataset.meta.camera_keys
        #     for cam in image_keys:
        #         item[cam] = self.transformations(item[cam])
...
def load_data(camera_names, chunk_size, config, rank0_print=print, policy_class=None, llava_pythia_process=None):
    ...
    for dataset in dataset_list:
      train_dataset_list.append(TransformedDataset(dataset, rank0_print=rank0_print, data_args=config['data_args']))
