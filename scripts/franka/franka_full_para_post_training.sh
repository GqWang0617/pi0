...
PRETRAIN=${ROOT}/model_param/multi_head2/${ACTION_HEAD}_results/checkpoint_all/${LLM}_${LLM_MODEL_SIZE}/vanilla_aloha_${LLM}_vla_pt_f_vit/qwen2_vl_3_cameras_1_17_all_data_pretrain_6w_DiT_H_Non_EMA_full_param_stage_1_50/checkpoint-60000 # with substeps DIT
...
OUTPUT=${ROOT}/model_param/multi_head2/${ACTION_HEAD}_results/checkpoint_all/${LLM}_${LLM_MODEL_SIZE}/vanilla_aloha_${LLM}_vla_pt_f_vit/${LLM}_3_cameras_only_fold_shirt_1_19_combine_constant_pretrain_Non_EMA_DIT_H_full_param_post_training_50_2w
if [ -d "$OUTPUT" ]; then
   echo 'output exists'
else
...
deepspeed --master_port 29604 --num_gpus=8 --num_nodes=1 ./train_vla.py \
  ...
  --lora_alpha 256 \
  --episode_first False \
  --task_name "3_cameras_only_folding_shirt_1_19" \
