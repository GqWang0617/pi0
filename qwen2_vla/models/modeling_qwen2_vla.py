...
class Qwen2VLForConditionalGenerationForVLA(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.ground_truth_reasoning = getattr(config, "ground_truth_reasoning", False)
        self.using_all_reasoning_hidden = getattr(config, "using_all_reasoning_hidden", False)
        ...
        if self.using_all_reasoning_hidden:
            self.gt_action_proj = ActionProjector(config.hidden_size, config.hidden_size)
            self.gt_film = FiLM(feature_dim=config.hidden_size, condition_dim=config.hidden_size)
        if self.using_film:
    ...
    def forward(
        ...
        if self.using_film:
            action_hidden_states = self.film_forward(labels=labels, input_ids=input_ids, prev_layer_hidden_states=outputs.hidden_states, hidden_states=hidden_states, ground_truth_reasoning_embed=ground_truth_reasoning_embed)
        elif self.using_xattn:
            action_hidden_states = self.xattn_forward(labels=labels, input_ids=input_ids, hidden_states=hidden_states)
        else: # tinyvla
            # action_hidden_states = []
            # inputs_index = labels[:, :] == -100 # -100 means do not learn
            ...
            #     action_hidden_states.append(torch.mean(h, dim=0))
            # action_hidden_states = torch.stack(action_hidden_states).unsqueeze(1)
    ...
    def film_forward(self, labels, input_ids, hidden_states, prev_layer_hidden_states=None, ground_truth_reasoning_embed=None):
        ...
        for i in range(indexs.shape[0]):
            ...
            if self.ground_truth_reasoning:
                gt_reasoning_embeddings.append(self.gt_action_proj(ground_truth_reasoning_embed[i, end:, :]))
        ...
        if self.using_all_reasoning_hidden:
            gt_reasoning_embeddings = torch.cat(gt_reasoning_embeddings, dim=0)
            # reasoning_embeddings = torch.cat([gt_reasoning_embeddings.unsqueeze(1), reasoning_embeddings.unsqueeze(1)],
            #                                  dim=1)
            # reasoning_embeddings = torch.mean(reasoning_embeddings, dim=1)
        elif self.ground_truth_reasoning:
            gt_reasoning_embeddings = torch.cat(gt_reasoning_embeddings, dim=0)
            reasoning_embeddings = gt_reasoning_embeddings

        action_hidden_states = self.reasoning_film(input_embeddings, reasoning_embeddings).unsqueeze(1)
        if self.using_all_reasoning_hidden:
            action_hidden_states = self.gt_film(action_hidden_states.squeeze(1), gt_reasoning_embeddings).unsqueeze(1)
    ...
    def evaluate(self,
        ...
        if self.using_film:
            action_hidden_states = self.film_forward(labels=labels,
                                                     ...
                                                     ground_truth_reasoning_embed=ground_truth_reasoning_embed)

        elif self.using_xattn:
            action_hidden_states = self.xattn_forward(labels=labels,
                                                     input_ids=torch.cat([input_ids, new_output_ids], dim=-1),
