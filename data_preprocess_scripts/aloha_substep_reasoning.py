...
def deal_subtask_label(path:str='/Volumes/PSSD-8/ljm/5_28_put_rubbish_can_succ_t0001_s-0-0/episode_12.hdf5', sub_reasonings=None, file_name=None, output=None, raw_lang=None, save_imgs=False, resize=True, using_substep_reasoning=True)->None:
    s1 = time.time()
    paths = path.split('/')
    ...
    if using_substep_reasoning:
        ...
        targets = sub_reasonings * data["action"].shape[0]
    if save_imgs:
        imgs = []
        ...
        for i,img in enumerate(data['observations']['images']['cam_high'][0::step]):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=16) as executor:
            for k, v in resized_imgs.items():
                try:
                    ...
                    results_list = list(executor.map(lambda img: _resize_image(img, k), data['observations']['images'][k][0::step]))
                except Exception as e:
                    print(f"Error occurred: {e}")

                resized_imgs[k] = results_list
    else:
        resized_imgs = {k: v for k,v in data['observations']['images'].items()}
        ...
        while stack:
            ...
            for name, item in cur_data.items():
                ...
                if isinstance(item, h5py.Group):
                    ...
                    cur_f.create_dataset(name, data=item[0::step])

        dest_f.create_dataset("substep_reasonings", data=targets[0::step])
...
def modify_subreasonings(path:str='/Volumes/PSSD-8/ljm/5_28_put_rubbish_can_succ_t0001_s-0-0/episode_12.hdf5', sub_reasonings=None, file_name=None, using_substep_reasoning=True, step=1)->None:
    s1 = time.time()
    paths = path.split('/')
    paths[-1] = f"low_res_{paths[-1]}"
    save_path = '/'.join(paths)
    ...
    data["substep_reasonings"][()]=targets[0::step]
    data['language_raw'][()] = ["Sorting the tablewares and rubbish on the table."]


def main1(task_require_substep_reasonings, raw_lang):
    ...
    for trsr, sub_r in task_require_substep_reasonings.items():
        ...
        r_l = raw_lang[trsr]
        if len(sub_r) == 1:
            using_substep_reasoning = False
        else:
            using_substep_reasoning = True
        i = 0
        for file_name in tqdm(file_list[:]):
            ...
            try:
                deal_subtask_label(path=file_path, sub_reasonings=sub_r, file_name=file_name, raw_lang=r_l,
                                   save_imgs=save_or_not, output=OUTPUT, resize=False,
                                   using_substep_reasoning=using_substep_reasoning)
            except Exception as e:
                print(e)
                print(trsr, file_name)
            i += 1

def main2(task_require_substep_reasonings, step=3):
    DATA_DIR = '/gpfs/private/tzb/wjj/data/aloha_bimanual/'
    for trsr, sub_r in tqdm(task_require_substep_reasonings.items()):
        file_path = os.path.join(DATA_DIR, trsr)
        file_list = os.listdir(file_path)
        file_list = [f for f in file_list if f.endswith('.hdf5')]
        file_list = sorted(file_list, key=lambda x: int(x.split('.')[0].split('_')[-1]))
        for file_name in tqdm(file_list[:]):
            ...
            modify_subreasonings(path=file_path, sub_reasonings=sub_r, file_name=file_name, step=1)
...
if __name__=="__main__":
    raw_lang = {
        ...
        'low_res_mug_red_can_paper_cup_paper': 'Sorting the tablewares and rubbish on the table.',
        'shovel_red_can_mug_orange_can': 'Sorting the tablewares and rubbish on the table.',
        '1106_2120_close_box': 'Can you help me pack these stuffs?'
    }
    ...
    task_require_substep_reasonings = {
        ...
        #     'waiting for next task.'
        # ],
        # 'low_res_low_res_mug_red_can_paper_cup_paper': [
        #     'pick mug with left hand and place it to left container.',
        #     'pick red can with right hand and place it to right rubbish bin.',
        #     'pick rubbish paper cup with left hand and place it to left container.',
        #     'pick crumpled paper ball with right hand and place it to right rubbish bin.',
        #     'waiting for next task.'
        # ],
        # 'shovel_red_can_mug_orange_can': [
        #     'pick shovel with left hand and place it to left container.',
        #     'pick red can with right hand and place it to right rubbish bin.',
        #     'pick mug with left hand and place it to left container.',
        #     'pick orange can with right hand and place it to right rubbish bin.',
        ...
        # ],
        # '1106_2120_close_box': [
        #     'Okay, the objects are already in box. Folding the box and close it.'
        # ]

        # process low_res reasonings on h100000000000000000000000000000000
        'low_res_low_res_mug_red_can_paper_cup_paper': [
            'pick mug with left hand and place it to left container.',
            'pick crumpled can with right hand and place it to right rubbish bin.',
            'pick paper cup trash with left hand and place it to left container.',
            'pick crumpled paper ball with right hand and place it to right rubbish bin.',
            'waiting for next task.'
        ],
        'low_res_bowl_red_can_brown_mug_purple_papercup': [
            'pick bowl with left hand and place it to left container.',
            'pick crumpled can with right hand and place it to right rubbish bin.',
            'pick mug with left hand and place it to left container.',
            'pick paper cup trash with right hand and place it to right rubbish bin.',
            'waiting for next task.'
        ],
        'low_res_plate_orange_can_bowl_paper': [
            'pick plate with left hand and place it to left container.',
            'pick crumpled can with right hand and place it to right rubbish bin.',
            'pick bowl with left hand and place it to left container.',
            'pick crumpled paper ball with right hand and place it to right rubbish bin.',
        ...
        'low_res_plate_red_can_bowl_paper': [
            'pick plate with left hand and place it to left container.',
            'pick crumpled can with right hand and place it to right rubbish bin.',
            'pick bowl with left hand and place it to left container.',
            'pick crumpled paper ball with right hand and place it to right rubbish bin.',
        ...
        'low_res_pot_blue_plastic_waste_shovel_green_paper_cup': [
            'pick pot with left hand and place it to left container.',
            'pick plastic waste with right hand and place it to right rubbish bin.',
            'pick shovel with left hand and place it to left container.',
            'pick paper cup trash with right hand and place it to right rubbish bin.',
            'waiting for next task.'
        ],
        'low_res_plate_paper_drink_box_pot_bowl': [
            'pick plate with left hand and place it to left container.',
            'pick paper drink box with right hand and place it to right rubbish bin.',
            'pick pot with left hand and place it to left container.',
        ...
        'low_res_shovel_red_can_mug_orange_can': [
            'pick shovel with left hand and place it to left container.',
            'pick crumpled can with right hand and place it to right rubbish bin.',
            'pick mug with left hand and place it to left container.',
            'pick crumpled can with right hand and place it to right rubbish bin.',
        ...
        'low_res_bowl_fenda_plate_paper': [
            'pick bowl with left hand and place it to left container.',
            'pick crumpled can with right hand and place it to right rubbish bin.',
            'pick plate with left hand and place it to left container.',
            'pick crumpled paper with right hand and place it to right rubbish bin.',
            'waiting for next task.'
        ]
    }
    # DATA_DIR = '/gpfs/private/tzb/wjj/data/aloha_bimanual/'
    ...
    # main1(task_require_substep_reasonings, raw_lang)
    main2(task_require_substep_reasonings)
